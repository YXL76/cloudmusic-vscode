...
const aesEncrypt = (buffer: Buffer, mode: string, key: Uint8Array | Buffer | string, iv: Buffer | string) => {
...
export const eapi = (url: string, object: Record<string, unknown>): { params: string } => {
  const text = JSON.stringify(object);
  const message = `nobody${url}use${text}md5forencrypt`;
  const digest = createHash("md5").update(message).digest("hex");
  const data = `${url}-36cd479b6b5-${text}-36cd479b6b5-${digest}`;
  return { params: aesEncrypt(Buffer.from(data), "ecb", eapiKey, "").toString("hex").toUpperCase() };
};
