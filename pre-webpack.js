...
const NeteaseCloudMusicApi = path.join(
  __dirname,
  "node_modules",
...
fs.writeFileSync(
  path.join(NeteaseCloudMusicApi, "module", "playlist_update.js"),
  `
...
fs.writeFileSync(
  path.join(NeteaseCloudMusicApi, "module", "song_detail.js"),
  `
module.exports = (query, request) => {
  const data = {
...
fs.writeFileSync(
  path.join(NeteaseCloudMusicApi, "module_types", "song_detail.d.ts"),
  `
...
fs.writeFileSync(
  path.join(NeteaseCloudMusicApi, "module_types", "base.d.ts"),
  `
...
fs.readdirSync(path.join(NeteaseCloudMusicApi, "module"))
  .reverse()
  .forEach((file) => {
    if (!file.endsWith(".js")) {
      return;
...
obj["${name}"] = function (data) {
  return ${name}(data, request);
};`);
  });

fs.writeFileSync(
  path.join(NeteaseCloudMusicApi, "main.js"),
  `
...
const cacache = path.join(__dirname, "node_modules", "@types", "cacache");

fs.writeFileSync(
  path.join(cacache, "en.d.ts"),
  `
